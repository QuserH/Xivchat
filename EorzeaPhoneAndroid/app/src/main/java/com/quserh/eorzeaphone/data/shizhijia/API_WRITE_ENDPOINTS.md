...
## 发帖 `posts/create` —— `scope` 不是帖子的可见范围（踩过）

body（官网 `PublishPost.DWDKRiEh.js`）：
...
| 字段 | 说明 |
|---|---|
| `id` / `updated_at` | **编辑**已有帖子才用，新发留空 |
| `type` | 1 帖子 / 2 攻略（同 postsList） |
| `part_id` | 版块 id，只能是叶子节点 |
| `is_share` | **分享到动态**开关，默认 0 |
| `scope` | **那条动态**的可见范围：1 公开 / 2 仅互关 / 3 仅自己 |
| `cover_pic` | 从正文 `<img src>` 提取，**排除 static.web.sdo.com**（那些是表情） |
| `has_vote` | 没有投票时 `"0"` |
| `atInfo` | 数组，**没有 @ 时整个键不发**（发 `"[]"` 会被拒："@信息格式不对"） |
...
**只有打开"分享到动态"时那三档才出现**，它约束的是那条动态。
**发到版块的帖子本身永远公开，石之家没有"私密帖"这回事。**

我一开始把它当帖子的可见性做进了发帖界面（标题写"谁能看"、含"仅自己可见"），
结果用户选了"仅自己可见"、以为发了私密帖，而帖子照常公开、别人能看到——
**这是个会误导人的错，不只是显示不对。** 界面上现在写明"帖子是公开的"，
scope 挂在"同时分享到动态"开关下面，关着就不出现。
