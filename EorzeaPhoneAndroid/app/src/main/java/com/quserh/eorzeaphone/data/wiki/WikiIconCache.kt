...
object WikiIconCache {
    private val mem = object : LruCache<String, Bitmap>(24_000) {
    ...
    suspend fun load(context: Context, iconHash: String): Bitmap? {
        ...
        mem.get(iconHash)?.let { return it }
        // 同一个 hash 只允许一个协程去下。
        ...
        // 显示了，「幻境利剑·蚀影」没显示。
        val lock = locks.getOrPut(iconHash) { Mutex() }
        return lock.withLock {
            mem.get(iconHash)?.let { return@withLock it }
            withContext(Dispatchers.IO) {
                val file = File(dir(context), "$iconHash.png")
                if (file.exists()) {
                ...
                val bmp = download(WikiRemote.huijiIconUrl(iconHash))
                    ?: return@withContext null
                // 先写临时文件再改名，避免别的协程读到写一半的内容
                runCatching {
                    val tmp = File(file.parentFile, "${iconHash}.tmp")
                    FileOutputStream(tmp).use {
                        bmp.compress(Bitmap.CompressFormat.PNG, 100, it)
                    }
                    if (!tmp.renameTo(file)) tmp.delete()
                }
                mem.put(iconHash, bmp)
                bmp
            }
        }
    }
